...
impl HnswNode{
    fn new(id:NodeId,embedding:Embedding,level:i64,neighbours:HashMap<i64,KDArray>)->HnswNode{
        return HnswNode{
            id,
            embedding,
            level,
...
impl HnswEngine{
    ...
    fn load(&mut self, chunks:Vec<String>){
        for i in chunks{
            ...
            while level>=0{
                ...
                self.update_neighbours_brute_force(&current_node_id,&level);
    ...
    fn traverse_max(&self,user_query:&String,k:&usize)->KSArray{
        let max_eq_search = usize::MAX;
        return self.traverse_core(user_query,k,&max_eq_search);
    }
